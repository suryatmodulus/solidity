...
void FileReader::setSource(boost::filesystem::path _fspath, SourceCode _source)
{
	auto sourceUnitID = _fspath.generic_string();
	m_sourceCodeOrigin[sourceUnitID] = boost::filesystem::path(sourceUnitID);
	m_sourceCodes[std::move(sourceUnitID)] = std::move(_source);
}

void FileReader::setSource(SourceUnitName _sourceUnitName, SourceCode _source)
{
	m_sourceCodes[_sourceUnitName] = std::move(_source);
}

void FileReader::setSource(SourceUnitName _sourceUnitName, optional<boost::filesystem::path> _fspath, SourceCode _source)
{
	if (_fspath.has_value())
		m_sourceCodeOrigin[_sourceUnitName] = _fspath.value();
	m_sourceCodes[std::move(_sourceUnitName)] = std::move(_source);
}

void FileReader::setSources(StringMap _sources)
{
	m_sourceCodeOrigin.clear(); // path mappings will be 1:1 from source unit ID to path.
